...
- **扩充 `kb-hard` 题型**：见「📊 评测」的实测结论——该题型目前只有 2 题，
  但它既是评测集**唯一有分辨力**的部分，也是最薄弱的一环。
...
精排占端到端耗时 99.6%（单次检索 **7124ms**，无精排 **389ms**）。
用环境变量 `RERANKER=noop` 做了对照实验（无需改代码）：

| 题型 | 路由符合率 | faithfulness | answer_rel | ctx_precision | ctx_recall |
| --- | --- | --- | --- | --- | --- |
| kb（6 题） | 1.00 → 1.00 | 0.97 → 0.99 | 0.99 → 0.97 | 1.00 → 1.00 | 1.00 → 1.00 |
| kb-hard（2 题） | **0.50 → 0.00** | — | — | — | — |
| rewrite（2 题） | 1.00 → 1.00 | 0.96 → 0.95 | 0.66 → 0.72 | 1.00 → 1.00 | 1.00 → 1.00 |

**结论**：

- **简单题（`kb`）不需要精排**：四列质量分无变化，差异全在 ±0.03 噪声范围内。
- **跨 chunk 题（`kb-hard`）依赖精排**：去掉后两道题**全部退化为联网检索**；
  保留精排时其中一道能在本地作答。
  → **精排的价值不在「让分数更高」，而在「让难题的本地检索够格、不退化成联网」**
  （联网意味着 +17s 延迟与外部 API 依赖）。
- **代价是 18 倍检索耗时**（389ms → 7124ms）。

> ⚠️ **这是信号，不是结论**：`kb-hard` 只有 2 题，真正翻转的只有 1 道。
> 且 `kb` 组四列质量分**已达 1.00（天花板效应）**，测不出差异是必然的。
> **整个评测集里真正有分辨力的只有那 2 道题** —— 这是评测集下一步该补的短板，
> 也是这次实验最硬的产出。
